<x-app-layout>
    <div class="py-12 bg-[#0b0c15] min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-[#1a1b26] overflow-hidden shadow-[0_0_20px_rgba(79,70,229,0.1)] sm:rounded-lg border border-gray-800 p-6 relative">
                
                <div class="mb-8 border-b border-gray-700 pb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                    ...
                    <div class="text-left md:text-right text-gray-400 text-sm">
                ...
                <div class="bg-[#0f1016] rounded-lg border border-gray-800 overflow-hidden mb-8">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center p-6 border-b border-gray-800 last:border-0 hover:bg-[#15161e] transition duration-300">
                        ...
                        <div class="flex items-center gap-5 w-full md:w-1/2 mb-6 md:mb-0">
                            ...
                            <div class="relative w-20 h-20 rounded-md overflow-hidden bg-gray-800 flex-shrink-0 border border-gray-700 shadow-md">
                                @if($imageSrc)
                        ...
                        <div class="w-full md:w-1/2 flex flex-col items-start md:items-end gap-3">
                            ...
                            @if($order->status == 'paid')
                                ...
                                <a href="{{ $item->product->download_url }}" target="_blank" class="w-full md:w-auto inline-flex items-center justify-center px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded text-sm group transition shadow-[0_0_15px_rgba(79,70,229,0.3)] hover:shadow-[0_0_20px_rgba(79,70,229,0.5)]">
                                ...
                                <div class="relative w-full md:w-80 group">
                                    <div class="flex items-center bg-[#050507] border border-gray-700 rounded overflow-hidden">
                                        ...
                                        <input type="text" id="link-{{ $item->id }}" value="{{ $item->product->download_url }}" readonly 
                                               class="w-full bg-transparent border-none text-gray-400 text-xs py-2 focus:ring-0 cursor-text font-mono truncate">
                                        
                                        <button onclick="copyToClipboard('link-{{ $item->id }}')" class="bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs font-bold px-4 py-2.5 transition border-l border-gray-700 flex items-center gap-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" /></svg>
                                            COPY
                                        </button>
                                    </div>
                                    <div id="msg-{{ $item->id }}" class="absolute right-0 -bottom-5 text-[10px] text-green-400 font-bold opacity-0 transition-opacity duration-300">
                                        Link Copied!
                                    </div>
                                </div>
                                ...
                                @if($existingReview)
                                    <div class="flex items-center gap-2 bg-[#252630] px-3 py-1.5 rounded border border-gray-700 mt-1">
                                        <span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Your Rating:</span>
                                        <div class="flex text-yellow-500">
                                            @for($i=1; $i<=5; $i++)
                                                <svg class="w-3 h-3 {{ $i <= $existingReview->rating ? 'fill-current' : 'text-gray-600 fill-current' }}" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
                                            @endfor
                                        </div>
                                    </div>
                                @else
                                    <button onclick="openReviewModal('{{ $item->product->id }}', '{{ $item->product->name }}')" 
                                            class="w-full md:w-auto mt-1 inline-flex items-center justify-center px-4 py-1.5 border border-yellow-600/30 text-yellow-500 hover:bg-yellow-600/10 font-bold rounded text-xs transition">
                            ...
                            @elseif($order->status == 'pending')
                                <div class="text-right">
                                    <span class="block text-xs text-yellow-500 font-bold bg-yellow-900/20 px-3 py-1.5 rounded border border-yellow-500/30 mb-1">
                                    ...
                                    <span class="text-[10px] text-gray-500">Pay to unlock access</span>
                                </div>
                            @else
                                <span class="text-xs text-red-500 font-bold bg-red-900/20 px-3 py-1.5 rounded border border-red-500/30">Order Cancelled</span>
                            @endif
                        </div>
                ...
                <div class="mt-8 flex justify-between items-center border-t border-gray-700 pt-6">
                    <a href="{{ route('orders.index') }}" class="text-gray-400 hover:text-white font-bold text-sm transition flex items-center gap-1 group">
    ...
    <div id="reviewModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-900 bg-opacity-80 transition-opacity" onclick="closeReviewModal()"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div class="inline-block align-bottom bg-[#1a1b26] rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full border border-gray-700">
                <form id="reviewForm" action="" method="POST" class="p-6">
                    @csrf
                    <div class="flex justify-between items-center mb-4 border-b border-gray-700 pb-2">
                        <h3 class="text-lg leading-6 font-bold text-white brand-font" id="modal-title">RATE: <span id="modalProductName" class="text-indigo-400">Product</span></h3>
                        <button type="button" onclick="closeReviewModal()" class="text-gray-400 hover:text-white"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
                    </div>
                    <div class="mt-4">
                        <div class="mb-6 text-center">
                            <label class="block text-gray-400 text-xs font-bold mb-2 uppercase tracking-widest">Rating</label>
                            <div class="flex flex-row-reverse justify-center gap-2 group">
                                @for($i=5; $i>=1; $i--)
                                    ...
                                    <label for="m_star{{$i}}" class="cursor-pointer text-gray-700 peer-checked:text-yellow-500 peer-hover:text-yellow-400 hover:text-yellow-400 transition transform hover:scale-125">
                        ...
                        <div class="mb-4">
                            <label class="block text-gray-400 text-xs font-bold mb-2 uppercase tracking-widest">Your Review</label>
                            <textarea name="comment" rows="4" class="w-full bg-[#0f1016] border border-gray-700 text-white rounded p-3 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 placeholder-gray-600 focus:outline-none"></textarea>
                        </div>
                    </div>
                    <div class="mt-6 flex justify-end gap-3">
                        <button type="button" onclick="closeReviewModal()" class="px-4 py-2 bg-transparent border border-gray-600 text-gray-300 rounded hover:text-white hover:border-gray-400 text-sm font-bold transition">CANCEL</button>
